<template>
    <transition name="fade">
        <div class="pa-8">
            <v-data-table :headers="headers" :loading="loading" class="elevation-3">
                <template v-slot:top>
                    <v-toolbar flat>
                        <v-toolbar-title
                            ...
                            films</v-toolbar-title
                        >
                        <v-divider class="mx-4" inset vertical></v-divider>
                        <v-spacer></v-spacer>
                        <v-dialog v-model="dialog" max-width="500px">
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on"> New films </v-btn>
                            </template>
                            <v-card>
                                <v-card-title>
                        ...
                        <v-dialog v-model="dialogDelete" max-width="500px">
                            <v-card>
                                <v-card-title class="text-h5">Are you sure you want to delete this films?</v-card-title>
                                <v-card-actions>
                                    <v-spacer></v-spacer>
                                    <v-btn color="blue darken-1" text>Cancel</v-btn>
...
export default {
    ...
    computed: {
        formTitle() {
            return this.editedIndex === -1 ? "New films" : "Edit films";
        },
    },
